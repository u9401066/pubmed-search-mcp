...
| 模式 | 啟動方式 | 工具面 | 適用情境 |
| --- | --- | --- | --- |
| Full schema + compatibility | `uv run python run_server.py --transport streamable-http --copilot-compatible` | 完整 46-tool primary MCP surface | 先嘗試保留完整功能 |
...
完整模式下，Copilot Studio 看到的是目前 server registry 的 primary MCP surface，也就是 46 個公開 tools。
