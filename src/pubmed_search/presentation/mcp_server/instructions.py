...
═══════════════════════════════════════════════════════════════════════════
🖼️ 圖片搜尋注意事項 (IMPORTANT for image search)
═══════════════════════════════════════════════════════════════════════════

### 設計原則: MCP 偵測，Agent 翻譯
⚠️ MCP 只能偵測非英文查詢並回傳錯誤提示，**不包含翻譯能力**。
翻譯是 Agent 的職責 (你有 LLM 翻譯能力)。MCP 絕不 hardcode 翻譯字典。

### 規則 1: Open-i 僅支援英文查詢 (最常見問題)
當 search_biomedical_images 回傳 non_english_detected 錯誤時，
你 (Agent) 必須用 LLM 能力翻譯成英文，然後重新搜尋。
...
### 規則 2: 圖片搜尋工作流
```
用戶要求圖片 → search_biomedical_images(query)
  ├─ 成功: 回傳結果
  └─ non_english_detected 錯誤: Agent 翻譯 → 重新搜尋
```

### 規則 3: 以圖搜文/以圖搜圖工作流
```
1. analyze_figure_for_search(url="...")  → 取得圖片
2. 你 (Agent) 用 Vision 能力分析圖片內容
3. 提取英文醫學關鍵字
4. 自動呼叫 search_biomedical_images() 或 unified_search()
5. 不要等用戶確認，直接執行搜尋並回傳結果
```
