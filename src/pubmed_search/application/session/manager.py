...
class ArticleCache:
    ...
    def _save_cache(self):
        ...
        try:
            data = {
                pmid: asdict(article) for pmid, article in self._memory_cache.items()
            }
    ...
    def stats(self) -> Dict[str, Any]:
        ...
        expired = sum(
            1 for a in self._memory_cache.values() if a.is_expired(self.max_age_days)
        )
        return {
            "total_cached": total,
            "valid": total - expired,
...
class SessionManager:
    ...
    def create_session(self, topic: str = "") -> ResearchSession:
        ...
        """
        session_id = hashlib.md5(  # noqa: S324  # nosec B324
        ...
        for record in reversed(session.search_history):  # Most recent first
            if record["query"].strip().lower() == normalized_query:
                ...
                if cached_articles and (not limit or len(cached_articles) >= limit):
                    logger.info(
                        f"Cache hit for query '{query}': {len(cached_articles)} articles"
                    )
