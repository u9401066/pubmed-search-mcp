...
def narrative_citation(entry: ChronicleEntry) -> str:
    ...
    entry_id = escape_markdown_text(entry.entry_id)
    ids = [escape_markdown_text(article.evidence_id) for article in entry.evidence.all_articles]
    return f"[{entry_id}; {', '.join(ids)}]" if ids else f"[{entry_id}; no evidence]"


def narrative_claim(entry: ChronicleEntry) -> str:
    """Render one evidence-backed list item from untrusted Chronicle fields."""
    return f"- {escape_markdown_text(entry.summary_claim)} {narrative_citation(entry)}"
...
def narrate_chronicle(snapshot: ChronicleSnapshot, *, mode: str = "brief") -> str:
    ...
    for branch_index, branch in enumerate(snapshot.branches):
        ...
        lines.append(f"## {escape_markdown_text(branch.name)}")
        lines.append("")
        for entry in selected:
            lines.append(narrative_claim(entry))
