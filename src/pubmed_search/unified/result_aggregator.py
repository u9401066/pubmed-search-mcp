...
class ResultAggregator:
    ...
    def _deduplicate(
        ...
        for article in articles:
            ...
            if strategy in (DeduplicationStrategy.MODERATE, DeduplicationStrategy.AGGRESSIVE):
                normalized_title = self._normalize_title(article.title)
                if normalized_title and len(normalized_title) > 20:  # Skip very short titles
        ...
        for article in articles:
            ...
            if article.pmid:
                for dup in pmid_index.get(article.pmid, []):
                    if id(dup) not in [id(d) for d in duplicates] and id(dup) not in processed:
                        duplicates.append(dup)
    ...
    def _select_primary(self, articles: list[UnifiedArticle]) -> UnifiedArticle:
        ...
        def score(article: UnifiedArticle) -> tuple:
            # Count identifiers
            id_count = sum([
                1 for x in [article.pmid, article.doi, article.pmc,
                           article.openalex_id, article.s2_id, article.core_id]
                if x
            ])
            
            # Count metadata completeness
            meta_count = sum([
                1 for x in [article.abstract, article.journal, article.year,
    ...
    def _calculate_relevance(
        ...
        abstract_overlap = len(query_terms & abstract_terms) / len(query_terms) if query_terms else 0
        ...
        keywords_overlap = len(query_terms & keywords_terms) / len(query_terms) if query_terms else 0
        
        # Weighted combination (title most important)
        relevance = (
            title_overlap * 0.5 +
            abstract_overlap * 0.3 +
            keywords_overlap * 0.2
        )
    ...
    def _calculate_quality(
        ...
        completeness = sum([
            1 for x in [article.abstract, article.doi, article.journal,
                       article.volume, article.issue, article.pages, article.year]
            if x
        ]) / 7
