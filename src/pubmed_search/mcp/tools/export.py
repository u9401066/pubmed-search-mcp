...
def register_export_tools(mcp: FastMCP, searcher: LiteratureSearcher):
    ...
    @mcp.tool()
    def prepare_export(
        pmids: Union[str, List, int], format: str = "ris", include_abstract: bool = True
    ) -> str:
        """
        ...
        try:
            ...
            exported_text = export_articles(
                articles, format=format_lower, include_abstract=normalized_abstract
            )

            # For large exports, save to file and return path
            if len(pmid_list) > 20:
                file_path = _save_export_file(
                    exported_text, format_lower, len(pmid_list)
                )
                return json.dumps(
                    {
                        "status": "success",
