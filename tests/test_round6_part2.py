...
class TestFulltextDownloaderDownloadPdf:
    ...
    async def test_download_pdf_preferred_source(self):
        ...
        from pubmed_search.infrastructure.sources.fulltext_download import (
            FulltextDownloader, PDFLink, PDFSource
        )
        ...
        with patch.object(downloader, 'get_pdf_links', new_callable=AsyncMock) as mock_links:
            ...
            with patch.object(downloader, '_download_from_url', new_callable=AsyncMock) as mock_download:
                from pubmed_search.infrastructure.sources.fulltext_download import DownloadResult
                ...
                result = await downloader.download_pdf(
                    doi="10.1234/test",
                    preferred_source=PDFSource.EUROPE_PMC
                )
            ...
            mock_response.json.return_value = {
                "message": {
                    "link": [
                        ...
                        {"URL": "https://example.com/paper.xml", "content-type": "application/xml"}
                    ]
                }
            }
...
class TestOpenURLConfigExtended:
    ...
    async def test_get_builder_invalid_preset(self):
        ...
        config = OpenURLConfig(
            ...
            resolver_base="https://fallback.edu",
            enabled=True
        )
...
class TestFetchImageAsync:
    ...
    async def test_fetch_invalid_url(self):
        ...
        from pubmed_search.presentation.mcp_server.tools.vision_search import fetch_image_as_base64
...
class TestVisionToolsRegistrationExtended:
    ...
    async def test_register_analyze_figure(self):
        ...
        tools = {}
        mock_mcp = Mock()
        mock_mcp.tool = lambda: lambda func: (tools.__setitem__(func.__name__, func), func)[1]
