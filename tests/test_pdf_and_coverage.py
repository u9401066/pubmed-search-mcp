...
class TestPDFMixin:
    ...
    async def test_get_pmc_fulltext_url_found(self, pdf_searcher):
    ...
    async def test_get_pmc_fulltext_url_not_found(self, pdf_searcher):
    ...
    async def test_get_pmc_fulltext_url_error(self, pdf_searcher):
    ...
    async def test_download_pmc_pdf_success(self, pdf_searcher, tmp_path):
    ...
    async def test_download_pmc_pdf_no_pmc(self, pdf_searcher, tmp_path):
    ...
    async def test_download_pmc_pdf_not_pdf(self, pdf_searcher, tmp_path):
    ...
    async def test_download_pmc_pdf_error(self, pdf_searcher, tmp_path):
    ...
    async def test_download_pdf_bytes_success(self, pdf_searcher):
    ...
    async def test_download_pdf_bytes_no_pmc(self, pdf_searcher):
    ...
    async def test_download_pdf_with_save(self, pdf_searcher, tmp_path):
    ...
    async def test_get_pmc_id_found(self, pdf_searcher):
    ...
    async def test_get_pmc_id_not_found(self, pdf_searcher):
    ...
    async def test_get_pmc_id_error(self, pdf_searcher):
        """Test _get_pmc_id with API error."""
        with patch("pubmed_search.infrastructure.ncbi.pdf.Entrez.elink") as mock_elink:
            mock_elink.side_effect = Exception("API Error")

            # Should not raise, just return None
            await pdf_searcher._get_pmc_id("12345")
...
class TestICiteExtended:
    ...
    async def test_enrich_with_citations(self, icite_searcher):
        ...
        with patch.object(icite_searcher, "get_citation_metrics") as mock_metrics:
            ...
            result = await icite_searcher.enrich_with_citations(articles)

            assert len(result) == 2
            assert result[0].get("icite") is not None

    async def test_sort_by_citations(self, icite_searcher):
    ...
    async def test_sort_by_rcr(self, icite_searcher):
    ...
    async def test_filter_by_citations(self, icite_searcher):
        """Test filtering articles by citation thresholds."""
        articles = [
            {
                "pmid": "1",
                "icite": {"citation_count": 10, "relative_citation_ratio": 0.5},
            },
            {
                "pmid": "2",
                "icite": {"citation_count": 50, "relative_citation_ratio": 3.0},
            },
            {
                "pmid": "3",
                "icite": {"citation_count": 25, "relative_citation_ratio": 1.5},
            },
        ]
