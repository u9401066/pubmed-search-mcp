...
class TestSearchAlternateSource:
    ...
    async def test_semantic_scholar(self, mock_get):
...
class TestCrossSearch:
    ...
    async def test_default_sources(self, mock_search):
        mock_search.return_value = [{"title": "Paper", "_source": "semantic_scholar"}]
        result = await cross_search("test query")
    ...
    @patch("pubmed_search.infrastructure.sources.search_alternate_source")
    async def test_pubmed_skipped(self, mock_search):
        mock_search.return_value = []
        result = await cross_search("test", sources=["pubmed", "openalex"])
        # pubmed should be skipped (handled elsewhere)
        assert "pubmed" not in result["by_source"]

    @patch("pubmed_search.infrastructure.sources.search_alternate_source")
    async def test_deduplication(self, mock_search):
        # Return same paper from two sources
        mock_search.side_effect = [
            [{"title": "Same Paper", "doi": "10.1/x", "_source": "ss"}],
            [{"title": "Same Paper", "doi": "10.1/x", "_source": "oa"}],
        ]
        result = await cross_search("test", sources=["semantic_scholar", "openalex"])
        assert result["stats"]["total"] == 1

    @patch("pubmed_search.infrastructure.sources.search_alternate_source")
    async def test_no_deduplication(self, mock_search):
        mock_search.side_effect = [
            [{"title": "Same Paper", "doi": "10.1/x", "_source": "ss"}],
            [{"title": "Same Paper", "doi": "10.1/x", "_source": "oa"}],
        ]
    ...
    @patch("pubmed_search.infrastructure.sources.search_alternate_source")
    async def test_source_exception(self, mock_search):
        mock_search.side_effect = Exception("network error")
        result = await cross_search("test", sources=["semantic_scholar"])
...
class TestDeduplicateResults:
    ...
    async def test_unique_papers(self):
        results = [
...
class TestNormalizeTitle:
    ...
    async def test_remove_punctuation(self):
        assert _normalize_title("Hello, world!") == "hello world"

    async def test_collapse_spaces(self):
        assert _normalize_title("hello   world") == "hello world"
...
class TestGetPaperFromAnySource:
    @patch("pubmed_search.infrastructure.sources.get_semantic_scholar_client")
    async def test_doi(self, mock_get):
        mock_client = AsyncMock()
        mock_client.get_paper.return_value = {"title": "DOI Paper"}
        mock_get.return_value = mock_client

        result = await get_paper_from_any_source("10.1234/test")
        assert result["title"] == "DOI Paper"

    @patch("pubmed_search.infrastructure.sources.get_openalex_client")
    @patch("pubmed_search.infrastructure.sources.get_semantic_scholar_client")
    async def test_doi_fallback_to_openalex(self, mock_ss, mock_oa):
    ...
    @patch("pubmed_search.infrastructure.sources.get_semantic_scholar_client")
    async def test_pmid(self, mock_get):
        mock_client = AsyncMock()
        mock_client.get_paper.return_value = {"title": "PMID Paper"}
        mock_get.return_value = mock_client

        result = await get_paper_from_any_source("12345678")
        assert result["title"] == "PMID Paper"

    @patch("pubmed_search.infrastructure.sources.get_semantic_scholar_client")
    async def test_pmid_prefix(self, mock_get):
        mock_client = AsyncMock()
        mock_client.get_paper.return_value = {"title": "PMID Paper"}
        mock_get.return_value = mock_client

        result = await get_paper_from_any_source("PMID:12345678")
        assert result is not None

    @patch("pubmed_search.infrastructure.sources.get_semantic_scholar_client")
    async def test_s2_id(self, mock_get):
        mock_client = AsyncMock()
        mock_client.get_paper.return_value = {"title": "S2 Paper"}
        mock_get.return_value = mock_client

        s2_id = "a" * 40
        result = await get_paper_from_any_source(s2_id)
        assert result["title"] == "S2 Paper"

    @patch("pubmed_search.infrastructure.sources.get_openalex_client")
    async def test_openalex_id(self, mock_get):
        mock_client = AsyncMock()
        mock_client.get_work.return_value = {"title": "OA Work"}
        mock_get.return_value = mock_client

        result = await get_paper_from_any_source("W123456")
        assert result["title"] == "OA Work"

    async def test_unknown_format(self):
        result = await get_paper_from_any_source("unknown!@#$")
...
class TestFulltext:
    ...
    async def test_get_fulltext_xml(self, mock_get):
        ...
        result = await get_fulltext_xml("PMC123")
        assert result == "<article>...</article>"

    @patch("pubmed_search.infrastructure.sources.get_europe_pmc_client")
    async def test_get_fulltext_parsed(self, mock_get):
        mock_client = AsyncMock()
        mock_client.get_fulltext_xml.return_value = "<article>test</article>"
        mock_client.parse_fulltext_xml = MagicMock(return_value={"title": "Parsed"})
        mock_get.return_value = mock_client

        result = await get_fulltext_parsed("PMC123")
        assert result["title"] == "Parsed"

    @patch("pubmed_search.infrastructure.sources.get_europe_pmc_client")
    async def test_get_fulltext_parsed_no_xml(self, mock_get):
        mock_client = AsyncMock()
