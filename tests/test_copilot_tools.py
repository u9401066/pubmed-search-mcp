...
class TestInputNormalizerPmids:
    ...
    def test_normalize_pmids_space_separated(self):
        """Test space-separated PMIDs."""
        result = InputNormalizer.normalize_pmids("12345678 87654321")
        assert result == ["12345678", "87654321"]

    def test_normalize_pmids_with_prefix(self):
        """Test with PMID: prefix."""
        result = InputNormalizer.normalize_pmids("PMID:12345678")
        assert result == ["12345678"]

    def test_normalize_pmids_mixed_prefixes(self):
        """Test mixed formats with prefixes."""
        result = InputNormalizer.normalize_pmids("PMID: 12345678, pubmed:87654321")
        assert result == ["12345678", "87654321"]

    def test_normalize_pmids_list_input(self):
        """Test list input."""
        result = InputNormalizer.normalize_pmids(["12345678", "87654321"])
        assert result == ["12345678", "87654321"]

    def test_normalize_pmids_int_list(self):
        """Test list of integers."""
        result = InputNormalizer.normalize_pmids([12345678, 87654321])
        assert result == ["12345678", "87654321"]

    def test_normalize_pmids_single_int(self):
        """Test single integer."""
        result = InputNormalizer.normalize_pmids(12345678)
        assert result == ["12345678"]

    def test_normalize_pmids_last_keyword(self):
        """Test 'last' keyword."""
        result = InputNormalizer.normalize_pmids("last")
        assert result == ["last"]

    def test_normalize_pmids_last_case_insensitive(self):
        """Test 'LAST' keyword."""
        result = InputNormalizer.normalize_pmids("LAST")
        assert result == ["last"]

    def test_normalize_pmids_semicolon_separated(self):
        """Test semicolon-separated PMIDs."""
        result = InputNormalizer.normalize_pmids("12345678;87654321")
        assert result == ["12345678", "87654321"]

    def test_normalize_pmids_pipe_separated(self):
...
class TestInputNormalizerPmidSingle:
    ...
    def test_normalize_pmid_single_valid(self):
        """Test valid single PMID."""
        result = InputNormalizer.normalize_pmid_single("12345678")
        assert result == "12345678"

    def test_normalize_pmid_single_with_prefix(self):
        """Test with prefix."""
        result = InputNormalizer.normalize_pmid_single("PMID:12345678")
        assert result == "12345678"

    def test_normalize_pmid_single_int(self):
        """Test integer input."""
        result = InputNormalizer.normalize_pmid_single(12345678)
        assert result == "12345678"

    def test_normalize_pmid_single_none(self):
        """Test None input."""
        result = InputNormalizer.normalize_pmid_single(None)
        assert result is None

    def test_normalize_pmid_single_empty(self):
        """Test empty string."""
...
class TestInputNormalizerPmcid:
    ...
    def test_normalize_pmcid_with_colon(self):
        """Test PMCID: prefix."""
        result = InputNormalizer.normalize_pmcid("PMCID: PMC7096777")
        assert result == "PMC7096777"

    def test_normalize_pmcid_none(self):
        """Test None input."""
...
class TestInputNormalizerYear:
    ...
    def test_normalize_year_str(self):
        """Test string year."""
        result = InputNormalizer.normalize_year("2024")
        assert result == 2024

    def test_normalize_year_with_suffix(self):
        """Test year with suffix."""
        result = InputNormalizer.normalize_year("2024年")
        assert result == 2024

    def test_normalize_year_with_prefix(self):
        """Test 'since' prefix."""
        result = InputNormalizer.normalize_year("since 2020")
        assert result == 2020

    def test_normalize_year_before(self):
        """Test 'before' prefix."""
    ...
    def test_normalize_year_invalid_range(self):
        """Test year outside valid range."""
        result = InputNormalizer.normalize_year(1800)
        assert result is None

    def test_normalize_year_future(self):
...
class TestInputNormalizerLimit:
    ...
    def test_normalize_limit_string(self):
        """Test string limit."""
        result = InputNormalizer.normalize_limit("25")
        assert result == 25

    def test_normalize_limit_min_clamp(self):
        """Test minimum clamping."""
        result = InputNormalizer.normalize_limit(0, min_val=1)
        assert result == 1

    def test_normalize_limit_max_clamp(self):
        """Test maximum clamping."""
        result = InputNormalizer.normalize_limit(200, max_val=100)
        assert result == 100

    def test_normalize_limit_invalid_string(self):
        """Test invalid string."""
...
class TestInputNormalizerBool:
    ...
    def test_normalize_bool_str_true(self):
        """Test 'true' string."""
        assert InputNormalizer.normalize_bool("true") is True

    def test_normalize_bool_str_yes(self):
        """Test 'yes' string."""
        assert InputNormalizer.normalize_bool("yes") is True

    def test_normalize_bool_str_1(self):
        """Test '1' string."""
        assert InputNormalizer.normalize_bool("1") is True

    def test_normalize_bool_str_false(self):
        """Test 'false' string."""
        assert InputNormalizer.normalize_bool("false") is False

    def test_normalize_bool_str_no(self):
        """Test 'no' string."""
        assert InputNormalizer.normalize_bool("no") is False

    def test_normalize_bool_int_1(self):
        """Test integer 1."""
        assert InputNormalizer.normalize_bool(1) is True

    def test_normalize_bool_int_0(self):
        """Test integer 0."""
        assert InputNormalizer.normalize_bool(0) is False

    def test_normalize_bool_none_default(self):
        """Test None with default."""
        assert InputNormalizer.normalize_bool(None, default=True) is True

    def test_normalize_bool_on_off(self):
...
class TestInputNormalizerQuery:
    ...
    def test_normalize_query_strip(self):
        """Test whitespace stripping."""
        result = InputNormalizer.normalize_query("  cancer treatment  ")
        assert result == "cancer treatment"

    def test_normalize_query_unicode_quotes(self):
...
class TestInputNormalizerIdentifier:
    ...
    def test_normalize_identifier_doi(self):
        """Test DOI detection."""
    ...
    def test_normalize_identifier_int(self):
        """Test integer input."""
        result = InputNormalizer.normalize_identifier(12345678)
        assert result["type"] == "pmid"
        assert result["value"] == "12345678"
...
class TestResponseFormatterNoResults:
    ...
    def test_no_results_with_alternatives(self):
...
class TestResponseFormatterPartialSuccess:
    ...
    def test_partial_success_basic(self):
    ...
    def test_partial_success_with_message(self):
    ...
    def test_partial_success_many_failures(self):
...
class TestKeyAliases:
    ...
    def test_apply_year_aliases(self):
...
class TestCommonFunctions:
    ...
    def test_set_get_session_manager(self):
    ...
    def test_set_get_strategy_generator(self):
...
class TestCopilotToolsIntegration:
    ...
    def test_register_tools(self, mock_mcp, mock_searcher):
    ...
    def test_search_pubmed_tool(self, mock_mcp, mock_searcher):
        ...
        if search_tool:
            result = search_tool("cancer treatment", limit=10)
            assert "Test Article" in result

    def test_get_article_tool(self, mock_mcp, mock_searcher):
        ...
        if get_article_tool:
            result = get_article_tool("12345678")
            assert "Test Article" in result

    def test_get_article_invalid_pmid(self, mock_mcp, mock_searcher):
        ...
        get_article_tool = mock_mcp._tools.get("get_article")
        if get_article_tool:
            result = get_article_tool("")
            assert "Invalid" in result or "Error" in result

    def test_find_related_tool(self, mock_mcp, mock_searcher):
        ...
        if find_related_tool:
            result = find_related_tool("12345678", limit=10)
...
class TestCopilotToolCount:
    ...
    def test_copilot_tool_count(self):
