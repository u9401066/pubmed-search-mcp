...
class TestParenthesesBalance:
    ...
    async def test_real_mesh_query(self):
        r = self.v.validate(
            '("Aspirin"[MeSH Terms]) AND ("Stroke"[MeSH Terms])'
        )
        assert r.is_valid is True
...
class TestFieldTagValidation:
    ...
    async def test_misspelled_tag(self):
        ...
        assert any("Invalid field tag" in e or "Unrecognized" in w
                    for e in r.errors for w in r.warnings) or \
               any("Invalid field tag" in e for e in r.errors)
    ...
    async def test_multiple_tags(self):
        r = self.v.validate(
            '"aspirin"[MeSH Terms] AND "stroke"[Title/Abstract]'
        )
        assert r.is_valid is True

    async def test_all_standard_tags_valid(self):
        """Spot-check that standard field tags don't raise errors."""
        for tag in ["Title", "tiab", "MeSH Terms", "pt", "dp", "la",
                     "au", "ta", "PMID", "filter", "sb", "1au"]:
...
class TestRealWorldQueries:
    ...
    async def test_mesh_expanded_query(self):
        ...
        q = (
            ...
            '"Insulin Resistance"[MeSH Terms]) AND '
            "(metformin treatment)"
        )
        r = self.v.validate(q)
        assert r.is_valid is True
    ...
    async def test_age_sex_species_filter(self):
        ...
        q = (
            'aspirin AND stroke AND "Adult"[MeSH] '
            'AND "Male"[MeSH] AND "Humans"[MeSH]'
        )
        r = self.v.validate(q)
        assert r.is_valid is True
