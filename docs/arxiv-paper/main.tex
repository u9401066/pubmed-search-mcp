...
The system exposes 45 Model Context Protocol (MCP) tools over a registry-backed broker for PubMed, Europe PMC, OpenAlex, Semantic Scholar, CORE, arXiv, medRxiv, bioRxiv, and optional licensed Scopus/Web of Science discovery, with Crossref enrichment, covering the complete pipeline from search to timeline.
...
The system is released as open-source software with 3{,}700+ automated tests.
...
Yet multi-step research workflows---involving agent-provided PICO handoff validation, parallel searches, result merging, and filtering---are ad hoc and not persistable.
...
\begin{enumerate}
    \item \textbf{Multi-source integration architecture:} An MCP SDK v2 system exposing 45 tools over registry-selected biomedical, scholarly-graph, open-access, preprint, and optional licensed sources through a Domain-Driven Design architecture (Section~\ref{sec:architecture}).
...
+-----------------------------------------------------------+
                   ...
                   MCP Protocol (JSON-RPC)
                          |
+-----------------------------------------------------------+
| Presentation Layer: 45 MCP Tools, 9 Prompts, Resources    |
+-----------------------------------------------------------+
| Application Layer                                         |
| +-- QueryAnalyzer (MeSH expansion, PICO handoff validation) |
...
| +-- Sem. Scholar   +-- CORE        +-- Preprint servers   |
| +-- Scopus/WoS*    +-- Crossref    +-- Unpaywall/Open-i   |
|     (* explicit, licensed opt-in)                         |
+-----------------------------------------------------------+
\end{verbatim}
...
Table~\ref{tab:sources} summarizes the current discovery and enrichment source families integrated by \system{}.

\begin{table}[htbp]
\centering
\caption{Registry-backed source contracts integrated by \system{}.}
\label{tab:sources}
\begin{tabular}{llll}
\toprule
\textbf{Source family} & \textbf{Role} & \textbf{Access} & \textbf{Provenance} \\
\midrule
PubMed/MEDLINE & Primary biomedical search & Email; key optional & Canonical PMID records \\
Europe PMC & Search and OA XML & Open & Aggregated records; hosted OA XML \\
OpenAlex/Semantic Scholar & Scholarly graph discovery & Optional keys & Indirect graph metadata \\
CORE & Open-repository discovery & Key recommended & Repository aggregator \\
arXiv/medRxiv/bioRxiv & Preprint discovery & Open & Native preprint records \\
Scopus/Web of Science & Licensed index discovery & Explicit keys and opt-in & Native index records \\
Crossref & DOI enrichment & Polite-pool email & Canonical DOI metadata \\
\bottomrule
\end{tabular}
\end{table}

The broker normalizes adapter outcomes into shared result envelopes and applies process-wide conservative provider budgets, source-specific concurrency, bounded timeouts, retry/backoff for retryable failures, and partial-failure reporting. Creating a second client does not create a second upstream quota.
...
\begin{itemize}
    \item \textbf{PICO Pipeline}: Agent-provided P/I/C/O $\rightarrow$ validation $\rightarrow$ parallel element searches $\rightarrow$ RRF merge
...
\system{} is implemented in Python using the MCP SDK v2 \texttt{MCPServer} API for MCP server capabilities.
...
The system exposes 45 MCP tools organized into 16 registry categories (Table~\ref{tab:tools}).
...
Query Intelligence & 3  & MeSH expansion, PICO handoff validation, query analysis \\
Article Exploration & 5 & Related articles, citations, references \\
Full Text          & 2  & Full-text retrieval and text mining \\
Figure Extraction  & 1  & PMC Open Access figure metadata \\
NCBI Extended      & 7  & Gene, PubChem, ClinVar databases \\
Citation Network   & 1  & Citation tree construction \\
Reference Verification & 1 & Citation-list verification \\
Export             & 2  & Citation files and literature notes \\
Session Management & 5  & Search history, artifacts, PMID caching \\
Institutional      & 5  & OpenURL and access diagnostics \\
ICD Conversion     & 1  & ICD-10 $\leftrightarrow$ MeSH mapping \\
Research Chronicle & 2  & Versioned timeline, lineage, and comparison \\
Image Search       & 1  & Biomedical image search \\
Visual Analysis    & 1  & Figure analysis for search \\
Pipeline           & 7  & Facade, persistence, history, and schedules \\
\bottomrule
\end{tabular}
\end{table}
...
The project maintains 3{,}700+ automated tests executed via pytest, including hermetic edge cases, protocol smoke tests, and opt-in live integration checks.
...
Our ranking pipeline---BM25$^{+}$ with field-level boosting, RRF multi-dimensional fusion, and MeSH-based MMR diversification---addresses the challenge of combining heterogeneous biomedical, scholarly-graph, open-access, preprint, and optional licensed sources without score calibration.
...
With 45 MCP tools, 3{,}700+ automated tests, and a registry-backed multi-source broker, \system{} demonstrates that the gap between literature \emph{search} and literature \emph{understanding} can be bridged through principled algorithms and thoughtful system design.

The system is open-source and available at \url{https://github.com/u9401066/pubmed-search-mcp}.
