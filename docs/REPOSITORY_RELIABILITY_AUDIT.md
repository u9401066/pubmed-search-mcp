...
由全庫測試覆蓋。本輪沒有新增 MCP 工具或變更公開參數；目前工具數維持 45。
...
原版 regression 對照使用先前評測已保存的 source archive，並核對 import 位置；
不修改目前工作區，也不修改原版 archive：
...
baseline = Path.home() / ".local/share/pubmed-search-mcp/benchmarks/papersearchqa-2026-09-09/assets/baseline-dbcf0c8"
sys.path.insert(0, str(baseline / "src"))
from pubmed_search.shared import cache_substrate
assert Path(cache_substrate.__file__).is_relative_to(baseline)
sys.exit(pytest.main(["-q", "tests/test_research_reliability.py"]))
PY
```
