...
[文件分類索引](#/documentation-map) · [維護腳本分類](https://github.com/u9401066/pubmed-search-mcp/blob/master/scripts/README.md)

研究 skill 安裝會保留既有自訂內容；請見[安裝與更新規則](#/troubleshooting#installing-research-skills-without-replacing-user-customizations)。開發者應在 push 前執行[本機完整驗證](https://github.com/u9401066/pubmed-search-mcp/blob/master/CONTRIBUTING.md#what-the-hooks-check)，一般 CI 僅保留獨立 smoke 檢查。

[十階段核心審查](reports/core_review_2026-09-15.md)與 [v0.7.4 複查](reports/release_v074_2026-09-18.md)涵蓋 `src/pubmed_search/` 全部 224 個 Python 檔案：458 個 class、2,456 個 function／method／nested function，合計 2,914 個定義。每項都有撰寫的自審理由、證據及目前檔案 SHA-256，沒有 pending 或過期紀錄；未變更的定義保留先前審查及來源紀錄。可執行 `uv run python scripts/perf/symbol_inventory.py --require-reviewed src/` 重新核對；後續修改會使受影響的審查失效。測試通過與語意審查分開計算，tests／scripts 不包含在這項核心全量完成宣告內。
...
共用併發採最保守的設定；429 冷卻不會隨逾時或 PubMed 背景任務取消而遺失。
歷史設計稿已歸檔，入口見[文件分類](#/documentation-map)。
詳見[發布複查](reports/release_v074_2026-09-18.md)與
[離線延遲量測](reports/search_execution_2026-09-18.md)；
固定延遲結果不能推論為真實 API 吞吐量或檢索品質提升。

## 核心審查與可靠性 — v0.7.3

已完成[十階段核心審查](reports/core_review_2026-09-15.md)：223 個原始碼檔案、2,901 個定義都有逐項理由與有效檔案雜湊。本版修正來源錯誤、文章識別、持久化、取消、匯出與評測 checkpoint，維持 41 個工具。安裝 harness 保留使用者自訂內容；push 前執行本機完整驗證，一般 CI 提供獨立 smoke。行為變更見 [CHANGELOG](https://github.com/u9401066/pubmed-search-mcp/blob/master/CHANGELOG.md#073---2026-09-15)；本機測試結果不代表新的公開 benchmark 增益。
...
[Benchmark 報告](#/academic-retrieval-benchmarks) 分開量測元件品質與完整
...
[可靠性稽核](#/repository-reliability-audit) 提供缺陷重現、驗證範圍與版本
...
先看[工具使用指南](#/tools-usage-guide-zh)：它把目前 41 個工具濃縮成 8 個能力族，說明理論上的最小壓縮邊界，以及人類與 agent 的意圖路由方式。
...
[開發者指南](#/developer-guide-zh#%E5%AE%8C%E6%95%B4-mcp-protocol-acceptance)。
