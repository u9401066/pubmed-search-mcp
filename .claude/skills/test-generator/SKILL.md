...
#### ⚠️ Async 測試規範 (本專案 CRITICAL)

本專案使用 `asyncio_mode = "auto"`。**每次新增測試後必須執行**：
...
規則：
- async 方法的 mock 必須用 `AsyncMock()`（不是 `Mock()`）
- 呼叫 async 方法必須加 `await`
- 測試 async 方法的函數必須是 `async def`
- 不需要 `@pytest.mark.asyncio`（`asyncio_mode = "auto"` 已自動偵測）
