...
其他工具給你原始 API 存取。我們給你**詞彙翻譯 + 智慧路由**：
...
4. **Agent-First 設計** - 輸出優化機器決策，非人類閱讀
...
│   專門搜尋（當需要特定來源時）                                      │
│                                                                  │
│   search_literature()       → 僅 PubMed（MeSH 支援）              │
│   search_europe_pmc()       → Europe PMC（全文/OA 篩選）          │
│   search_core()             → CORE 200M+ 開放取用                 │
│   search_gene()             → NCBI Gene 資料庫                    │
│   search_compound()         → PubChem 化合物                      │
│   search_clinvar()          → ClinVar 變異                        │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```
...
| 類別 | 工具 |
|------|------|
| **全文** | `get_fulltext` (Europe PMC), `get_core_fulltext` (CORE), `get_fulltext_xml` |
